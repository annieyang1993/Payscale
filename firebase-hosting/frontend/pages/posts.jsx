            ...
            <div className = 'under-header' style={{color: 'rgb(90, 90, 90)'}}>
                ...
                <div className = 'posts-left'>
                    {authContext.postsArr.map((post, i)=>{
                        if (i%2 === 0 && authContext.posts[post].live === true){
                            return(
                            <Link onClick={()=>{authContext.setCurrentPost(authContext.posts[post]); authContext.setCurrentPostSet(true)}} className = 'post-link' style={i%2 === 0 ? {marginRight: '9%'} : {}} to={`/posts/${authContext.posts[post].title.toLowerCase().split(' ').join('-')}?id=${post}`}>
                            ...
                            <br/><br/><br/>
                            <div style={{textAlign: 'center', width: '100%', color: 'gray', marginBottom: '20px', marginTop: '30px'}}>{new Date(authContext.posts[post].created_on.seconds*1000).toLocaleDateString()} - {authContext.posts[post].read_time} min read</div>
                            </Link>)
                        }
                ...
                <div className = 'posts-right'>
                    {authContext.postsArr.map((post, i)=>{
                        if (i%2 === 1 && authContext.posts[post].live === true){
                            return(
                            <Link onClick={()=>{authContext.setCurrentPost(authContext.posts[post]); authContext.setCurrentPostSet(true)}} className = 'post-link' style={i%2 === 0 ? {marginRight: '9%'} : {}} to={`/posts/${authContext.posts[post].title.toLowerCase().split(' ').join('-')}?id=${post}`}>
                            <img style={{width: '100%'}} src={authContext.posts[post].img}/>
                            <h1 style={{textAlign: 'center', width: '100%'}} className = 'posts-title'>{authContext.posts[post].title}</h1>
        ...
        <div className = 'content'>
            ...
            <div className = 'under-header' style={{color: 'rgb(90, 90, 90)'}}>
                ...
                <div className = 'posts-inner'>
                    ...
                    {authContext.postsArr.map((post, i)=>{
                        if (authContext.posts[post].live === true){
                            return(
                            <Link onClick={()=>{authContext.setCurrentPost(authContext.posts[post]); authContext.setCurrentPostSet(true)}} className = 'post-link' style={{marginRight: '0%', marginLeft: '0%', marginBottom: '50px'}} to={`/posts/${authContext.posts[post].title.toLowerCase().split(' ').join('-')}?id=${post}`}>
                            <img style={{width: '100%'}} src={authContext.posts[post].img}/>
                            <h1 style={{textAlign: 'center', width: '100%'}} className = 'posts-title'>{authContext.posts[post].title}</h1>
                            <div style={{textAlign: 'center', width: '100%', color: 'gray', marginBottom: '20px;'}}>{authContext.posts[post].description}</div>
                            {/* <Markup content={authContext.posts[post].post}/> */}
